    ...
    */
    if (socket)
    {
      socket.once('data', function(data) {
        ...
        console.log('realIP', socket.remoteAddress)
        if (searchPos === -1) {
          console.log('header', self.header, 'not found.')
          var dataArr = strData.split(' ')
          var originIP
          if (dataArr[0] === 'proxy' && dataArr.length >= 2) {
            originIP = dataArr[1] + ':' + dataArr[2]
          }
          ...
          if (!originIP) {
            ...
            return process.send(
              { cmd: 'sticky-session:ack', realIP: originIP, data: data },
              socket
            );
